/*
*  Course:      CSC 111
*  Author:      Michael Reiter and B. Bird
*  UVicId:      V00831568
*  Assignment:  A7P1
*  Date:        28/10/14
...
void process_wireframe_model(FILE* inputFile, FILE* outputFile){
	...
	Matrix M, P, T, S, R_X, R_Z, PT, PTS, PTSR_X;

	projectionMatrix(P);
	translationMatrix(T, 400, 400, 400);
	scaleMatrix(S, 700, 700, 700);
	rotationMatrixX(R_X, 20*M_PI/180);
	rotationMatrixZ(R_Z, 330*M_PI/180);

	matMul(P, T, 2, 4, 4, PT);
	matMul(PT, S, 2, 4, 4, PTS);
	matMul(PTS, R_X, 2, 4, 4, PTSR_X);
	matMul(PTSR_X, R_Z, 2, 4, 4, M);
